...
function changeTabTitle(newSong) {
  const correctness = newSong.correct
    ? "✔️ "
    : newSong.correct === false
    ? "❌ "
    : "";
...
let answerResultsListener = new Listener("answer results", (result) => {
  ...
  setTimeout(() => {
    ...
    let findPlayer = Object.values(quiz.players).find((tmpPlayer) => {
      return (
        tmpPlayer._name === selfName && tmpPlayer.avatarSlot._disabled === false
      );
    });
    ...
    // support for Nexus
    if (result.players[0].score === null)
...
document
  .getElementById("qpLeaveButton")
  .addEventListener("click", () => resetTabTitle());
